...
function __construct() {
	...
	require_once(project."/controls/signin/url.php");
	$this->signinUrl=new signinUrl();
...
public static function request() {	
	...
	if (isset(signinUrl::$key)) {
		if (signinUrl::$key!='') {
			self::$key=signinUrl::$key;
			self::$type='key';
			return true;
		}
	}
	...
	if ((isset(signinUrl::$login)) && (isset(signinUrl::$password))) {
		if ((signinUrl::$login!='') && (signinUrl::$password!='')) {
			self::$login=signinUrl::$login;
			self::$password=signinUrl::$password;
			self::$type='login';
			return true;
		}
	}	
